    ...
    try {
        ...
        if (response.ok) {
            ...
            if (comicDataResponse.ok) {
                ...
                return { comic: null, error: "Failed to fetch comic details: " + comicDataResponse.status };
            }
        } else {
            return { comic: null, error: "Failed to find comic ID: " + response.status };
        }
    } catch (error) {
...
const ComicPage: React.FC<{ comic: Comic | null, error: string | null }> = ({ comic, error }) => {
    return (
        <div>
            <Header />
            <Menu />
